...
class SearchEngine:
    ...
    def _search_mit(self, query, max_results):
        """MIT arama"""
        return [{
            'title': f'MIT DSpace: {query}',
            'authors': 'MIT',
            'year': datetime.now().year,
            'source': 'MIT',
            'link': f'https://dspace.mit.edu/handle/1721.1/49402/browse?type=title&value={requests.utils.quote(query.upper())}'
        }]
...
class AcademicSearcherPro:
    ...
    def setup_search_controls(self):
        ...
        ttk.Label(title_frame, text="🔬 AKADEMİK MAKALELERİN GOOGLE'I", 
    ...
    def setup_source_selection(self):
        ...
        # Kaynakları iki sütuna yerleştir
        ...
        for i, source in enumerate(sources_list):
            frame = left_frame if i < mid else right_frame
    ...
    def setup_filters(self):
        ...
        ttk.Combobox(filter_frame, textvariable=self.max_results, 
                    values=['20', '50', '100'], width=8).pack(side='left')
    ...
    def select_tr_sources(self):
        ...
        tr_sources = ['DergiPark', 'TÜBİTAK', 'ODTÜ', 'İTÜ', 'Boğaziçi', 'Ankara Üniv.']
    ...
    def select_intl_sources(self):
        ...
        intl_sources = ['DOAJ', 'ArXiv', 'Crossref', 'PubMed', 'IEEE', 'MIT']
        for source, var in self.sources.items():
